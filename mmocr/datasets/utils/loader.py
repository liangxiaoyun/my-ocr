...
class LoaderParsertxt:
    ...
    def __init__(self, ann_file, separator=' ', data_root=None, keys=['filename', 'text'], repeat=1):
        assert isinstance(ann_file, str)
        ...
        with open(ann_file, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                if len(line) > 2:
                    ...
                    text = ''
                    for i in line[1:]:
                        text = text + i + self.separator
                    text = text[:-1]
                    line = [filename, text]
                if len(line) < 2 or len(line[1]) == 0:
                    continue
                if self.data_root is not None:
                    line[0] = self.data_root + '/' + line[0]
                if not osp.exists(line[0]):# or not osp.getsize(line[0]):
                    continue
    ...
    def __getitem__(self, index):
        ...
        for i, key in enumerate(self.keys):
            line_info[key] = self.ori_data_infos[index][i]
        return line_info
